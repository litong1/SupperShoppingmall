...
<body>
	...
	<div class="main" id="form-main">
		<form id="form"
			action="${pageContext.request.contextPath}/changeQuestionServlet?username=<%=request.getParameter("username") %>"
			method="post">
			<div class="msg">
			...
			<div class="form-item">
				<label>答案一：</label> 
				<textarea rows="1" cols="10" style="resize:none" id="question1" name="question1" ><%=list.size() == 1 ? "" : list.get(0).getAnswer() == null ? "" : list.get(0).getAnswer()%></textarea>
			</div>
			...
			<div class="form-item">
				<label>答案二：</label> 
				<textarea rows="1" cols="10" style="resize:none" id="question2" name="question2" ><%=list.size() == 1 ? "" : list.get(1).getAnswer() == null ? "" : list.get(1).getAnswer()%></textarea>
			</div>
			<label>密保三：你的第三任女朋友叫什么？</label>
			<div class="form-item">
				<label>答案三：</label>
				<textarea rows="1" cols="10" style="resize:none" id="question3" name="question3" ><%=list.size() == 1 ? "" : list.get(2).getAnswer() == null ? "" : list.get(2).getAnswer()%></textarea>
			</div>
		</form>
		<div>
